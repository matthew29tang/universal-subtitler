...
const callAPI = (key, url) => {
  let api =
    "https://uploadbeta.com/api/video/?cached&video=" +
...
function setUrlOffline(url, url2 = "") {
  if (url.includes("weibomiaopai.com")) {
    // alternative
    $("div#down").html(
      "<h3>" +
        get_text("videos_list") +
        "</h3><ul><li><a target=_blank rel=nofollow href='" +
        url +
        "'>" +
        "<i><font color=gray>" +
        url +
...
function setUrlOfflineArray(urls) {
  ...
  for (let i = 0; i < urls_length; ++i) {
    s +=
      ...
      urls[i] +
      "'>" +
      urls[i].TrimToLength(max_url_length) +
      "</a></li>";
  }
  s += "</ol>";
  $("div#down").html(s);
...
document.addEventListener(
  ...
  function() {
    ...
    chrome.tabs.getSelected(null, function(tab) {
      ...
      if (!domain.includes("youtube.com")) {
        ...
        if ($("select#lang").val() != "en-us") {
          ...
          s =
            "https://weibomiaopai.com/download-video-parser.php?url=" +
      ...
      $("#pic").click(function() {
        ...
        $.ajax({
          ...
          success: function(data) {
            let tmp = [];
            let re = /<img\s[^>]*?src\s*=\s*['\"]([^'\"]*?)['\"][^>]*?>/gi;
            let found = re.exec(data);
            while (found != null) {
              let tmp_url = found[1];
              if (tmp_url != null && tmp_url.length > 0) {
                if (
                  tmp_url.startsWith("http://") ||
                  tmp_url.startsWith("https://")
                ) {
                  tmp.push(tmp_url);
                } else {
                  if (tmp_url[0] == "/") {
      ...
      $("#links").click(function() {
        ...
        $.ajax({
          ...
          success: function(data) {
            let tmp = [];
            let re = /(https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&\/\/=]*))/gi;
            let found = re.exec(data);
            while (found != null) {
              let tmp_url = found[0];
              if (tmp_url != null && tmp_url.length > 0) {
    ...
    const url =
      "https://raw.githubusercontent.com/DoctorLai/VideoDownloadHelper/master/video-url-parser/tested-urls.txt";
