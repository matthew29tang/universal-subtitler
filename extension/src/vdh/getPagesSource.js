...
export function getPagesSource(){
  "use strict";
  ...
  if (domain.includes("weibo.com") || domain.includes("video.sina.com.cn")) {
    if (!ValidURL(video_url)) {
      video_dom = document.querySelector(
        "div[node-type='common_video_player']"
      );
      if (video_dom) {
  ...
  if (domain.includes("facebook.com")) {
    ...
    if (!ValidURL(video_url)) {
      let re = /['"]?sd_src_no_ratelimit['"]?: *(['"])(https?:[^\s'",]+)\1,/gi;
      let found = re.exec(html);
      while (found != null) {
        video_url = found[2];
        if (ValidURL(video_url)) {
    ...
    if (!ValidURL(video_url)) {
      ...
      if (ValidURL(tmp)) {
        ...
        function substr(param1, param2) {
          ...
          let loc4 = param1.substring(
            parseInt(param2[0]),
            parseInt(param2[0]) + parseInt(param2[1])
          );
  ...
  if (ValidURL(video_url)) {
    ...
    // sniffer!
    if (chrome.devtools) {
      // only available within a devltools page
